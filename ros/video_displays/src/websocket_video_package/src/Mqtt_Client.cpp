...
Mqtt_Client::Mqtt_Client(ros::NodeHandle& nh, MySocketServer& socket, std::string topicName_rgb)
    : nh_(nh)
    , mySocketServer(
          socket)
    , topicName_rgb(
...
void Mqtt_Client::callback_rgb(const sensor_msgs::ImageConstPtr& msg)
{

    std::cout << "收到图像" << std::endl;
