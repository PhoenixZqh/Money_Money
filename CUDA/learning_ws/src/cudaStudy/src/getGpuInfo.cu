...
int main(int argc, char** argv)
{
    printf("%s Starting ...\n", argv[0]);
    int deviceCount = 0;
    cudaError_t error_id = cudaGetDeviceCount(&deviceCount); // 返回具有计算能力的设备的数量，传入的是引用，会改变值
    ...
    cudaSetDevice(dev); // 设置用于GPU执行的设备
    ...
    printf("  CUDA Driver Version / Runtime Version         %d.%d  /  %d.%d\n",
        ...
        runtimeVersion / 1000, (runtimeVersion % 100) / 10); // 得到cuda_driver 和 cuda_runtime的版本
