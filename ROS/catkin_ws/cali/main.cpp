...
// int main()
// {
//     std::string dir = "/home/dym/cali/";

//     cv::Mat camera_K(3, 3, CV_32FC1, cv::Scalar::all(0));
//     camera_K.at<float>(0,0) = 709.5230216209841;
//     camera_K.at<float>(0,2) = 651.9985478532343;
//     camera_K.at<float>(1,1) = 711.3148104552893;
//     camera_K.at<float>(1,2) = 354.7380097173606;
//     camera_K.at<float>(2,2) = 1;

//     cv::Mat distCoeffs(1, 5, CV_32FC1, cv::Scalar::all(0));
//     distCoeffs.at<float>(0,0) = -0.3260526700650427;
//     distCoeffs.at<float>(0,1) = 0.1384733784434554;
//     distCoeffs.at<float>(0,2) = 0.0006465957645170084;
//     distCoeffs.at<float>(0,3) = -0.0002451373854745326;
//     distCoeffs.at<float>(0,4) = -0.03202322458328089;

//     int index = 0;

//     cv::Mat mapx, mapy;
//     cv::initUndistortRectifyMap(camera_K, distCoeffs, cv::Mat(), cv::Mat(),
//     cv::Size(1280,720), CV_32FC1, mapx, mapy);
...
//         cv::Mat dst;
//         cv::remap(src, dst, mapx, mapy, CV_INTER_LINEAR, cv::BORDER_CONSTANT,
//         cv::Scalar(0));
//         // std::vector<cv::Point2f> pts = {cv::Point2f(640.f,360.f),
//         cv::Point2f(240.f,180.f), cv::Point2f(120.f,90.f)};
//         // std::vector<cv::Point2f> dst;

//         // cv::undistortPoints(pts, dst, camera_K, distCoeffs, cv::noArray(),
//         camera_K);
...
// //函数声明,默认每行11个block, 没列8个block, block大小为75个像素.
// 也就是10*7个内点 void drawChessBoard(int blocks_per_row=11, int
// blocks_per_col=8, int block_size = 75);
...
//     cv::Size board_size = cv::Size(block_size * blocks_per_row, block_size *
//     blocks_per_col); cv::Mat chessboard = cv::Mat(board_size, CV_8UC1);
...
//         {
//             chessboard(cv::Rect(i * block_size, j * block_size, block_size,
//             block_size)).setTo(color); color = ~color;
//         }
//     }
//     cv::Mat chess_board = cv::Mat(board_size.height + 100, board_size.width +
//     100, CV_8UC1, cv::Scalar::all(256)); //上下左右留出50个像素空白
//     chessboard.copyTo(chess_board.rowRange(50, 50 +
//     board_size.height).colRange(50, 50 + board_size.width));
